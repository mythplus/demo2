"""
Mem0 Dashboard 后端 - 图谱记忆路由（Neo4j）
"""
import logging
...
async def _neo4j_query_async(cypher: str, params: dict = None) -> list:
    """异步执行 Neo4j Cypher 查询（通过 asyncio.to_thread 避免阻塞事件循环）"""
    import asyncio
    return await asyncio.to_thread(_neo4j_query, cypher, params)
...
async def get_graph_entities(
    ...
    try:
        ...
        """
        entities = await _neo4j_query_async(cypher, params)

        count_cypher = f"MATCH (n){where_str} RETURN count(n) as total"
        count_params = {k: v for k, v in params.items() if k not in ("limit", "offset")}
        total_result = await _neo4j_query_async(count_cypher, count_params)
...
async def get_graph_relations(
    ...
    try:
        ...
        return {"relations": relations, "total": total, "limit": limit, "offset": offset}
    except Exception as e:
...
async def search_graph(request: GraphSearchRequest):
    ...
    try:
        ...
        entity_cypher = f"""
            ...
            LIMIT $limit
        """
        isolated_entities = await _neo4j_query_async(entity_cypher, params)
...
async def get_user_graph(user_id: str, limit: int = Query(200, ge=1, le=1000)):
    """获取指定用户的子图数据"""
    try:
        cypher = """
            MATCH (a)-[r]->(b)
            ...
            LIMIT $limit
        """
        relations = await _neo4j_query_async(cypher, {"user_id": user_id, "limit": limit})
        ...
        for rel in relations:
            ...
            if source_name not in nodes_map:
                nodes_map[source_name] = {
                    ...
                    "labels": rel.get("source_labels", []), "val": 1,
                }
            else:
            ...
            if target_name not in nodes_map:
                nodes_map[target_name] = {
                    ...
                    "labels": rel.get("target_labels", []), "val": 1,
                }
            else:
                nodes_map[target_name]["val"] += 1

            links.append({"source": source_name, "target": target_name, "relation": rel["relation"]})
        ...
        isolated_cypher = """
            ...
            LIMIT $limit
        """
        isolated = await _neo4j_query_async(isolated_cypher, {"user_id": user_id, "limit": limit})
...
async def delete_graph_entity(entity_name: str, user_id: Optional[str] = Query(None)):
    """删除指定实体及其所有关联关系"""
    try:
        params: Dict[str, object] = {"name": entity_name}
        user_filter = ""
        if user_id:
        ...
        result = await _neo4j_query_async(cypher, params)
...
async def delete_graph_relation(
    ...
    try:
        ...
        """
        result = await _neo4j_query_async(cypher, {"source": source, "relation": relation, "target": target})
        deleted = result[0]["deleted"] if result else 0
