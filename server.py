...
async def _auto_categorize_memory(memory_text: str) -> List[str]:
...
# ============ 全局异步 HTTP 客户端（替代阻塞的 sync requests） ============
...
async def test_llm_connection():
    ...
    try:
        if provider == "ollama":
            ...
            resp = await _http_client.get(f"{base_url}/api/tags", timeout=10)
            ...
            # 进一步做一次简单的生成测试
            gen_resp = await _http_client.post(
            ...
            resp = await _http_client.get(f"{base_url}/models", headers=headers, timeout=10)
            resp.raise_for_status()
            return {
                "status": "connected",
                "provider": provider,
...
async def add_memory(request: AddMemoryRequest):
    ...
    try:
        ...
        if not user_selected_categories and request.auto_categorize:
            ...
            ai_categories = await _auto_categorize_memory(memory_text)
            if ai_categories:
                    ...
                    try:
                        ...
                        if points:
                            ...
                            if not user_selected_categories and request.auto_categorize and request.infer:
                                ...
                                if memory_content:
                                    per_item_cats = await _auto_categorize_memory(memory_content)
                                    if per_item_cats:
    ...
    async def _process_single_item(idx: int, item: BatchImportItem) -> BatchImportResultItem:
        ...
        async with _semaphore:
            try:
                ...
                # AI 自动分类（异步，不阻塞事件循环）
                if not user_selected_categories and request.auto_categorize:
                    ai_categories = await _auto_categorize_memory(item.content)
                    if ai_categories:
                        final_metadata["categories"] = ai_categories
...
async def batch_delete_memories(request: BatchDeleteRequest):
    ...
    for mid in request.memory_ids:
        ...
        old_memory_text = payload.get("data", "")
        old_categories = metadata.get("categories", [])
        _save_change_log(mid, "DELETE", old_memory_text, old_categories)
