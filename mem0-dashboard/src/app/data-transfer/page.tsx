...
export default function DataTransferPage() {
  ...
  // 获取筛选后的记忆数据（分页循环拉取，避免大数据量一次性加载导致超时/OOM）
  const getFilteredMemories = useCallback(async (
    ...
    // 第一页：获取 total 用于进度计算
    const firstPage = await mem0Api.getMemories({
    ...
    allMemories.push(...(firstPage.items || []));
    totalCount = firstPage.total || allMemories.length;
    onProgress?.(allMemories.length, totalCount);

    // 后续页
    const totalPages = Math.ceil(totalCount / PAGE_SIZE);
    for (page = 2; page <= totalPages; page++) {
      const pageData = await mem0Api.getMemories({
        user_id: filterUserId || undefined,
        date_from: filterDateFrom || undefined,
        date_to: filterDateTo || undefined,
        page,
  ...
  // 通用导出逻辑（带进度反馈 — 分页循环拉取，支持大数据量）
  const handleExport = async (format: "json" | "csv") => {
    setExporting(true);
    setExportProgress(0);
    setExportStage("正在获取数据...");
    try {
      // 阶段 1：分页循环获取数据（0% → 70%），进度实时更新
      setExportProgress(5);
      const progressCallback = (loaded: number, total: number) => {
        const pct = total > 0 ? Math.round((loaded / total) * 65) + 5 : 30;
        setExportProgress(Math.min(pct, 70));
        setExportStage(`正在获取数据... (${loaded}/${total})`);
      };
      const data = await getFilteredMemories(progressCallback);
